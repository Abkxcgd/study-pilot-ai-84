...
export async function seedDemoData() {
  ...
  const tasks = [
    ...
    {
      title: "Review Machine Learning lecture",
      priority: "medium",
      status: "todo",
      due_date: addDays(3),
    },
    ...
    {
      title: "Prepare notes for presentation",
      priority: "low",
      status: "todo",
      due_date: addDays(5),
    },
  ...
  const notes = [
    {
      ...
      source_text:
        "High bias means the model is too simple; high variance means it overfits. Regularization (L1, L2) reduces variance. Cross-validation helps estimate generalization error.",
      summary:
        "Bias/variance tradeoff explains under- vs overfitting. Use regularization and cross-validation.",
      key_points: [
        "Bias = underfitting",
    ...
    },
    {
      ...
      source_text:
        "Arrays: O(1) access, O(n) search. HashMap: O(1) avg lookup. BST: O(log n) balanced. Sorting: mergesort O(n log n).",
    ...
    },
    {
      ...
      source_text:
        "1NF: atomic values. 2NF: no partial dependency. 3NF: no transitive dependency. BCNF: stricter 3NF.",
