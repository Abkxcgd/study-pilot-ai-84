...
function ExamPage() {
  ...
  const start = async () => {
    ...
    try {
      const res = await run({
        data: { topic, difficulty, count, sourceText: sourceText || undefined },
      });
  ...
  const correctCount = submitted
    ? questions.reduce((n, q, i) => n + (answers[i] === q.answerIndex ? 1 : 0), 0)
    : 0;
  ...
  const weakTopics = submitted
    ? Array.from(
        new Set(questions.filter((q, i) => answers[i] !== q.answerIndex).map((q) => q.topic)),
      )
    : [];
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Adaptive mock tests with instant scoring & weak-topic detection.
          </p>
        </div>
      </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
            ...
            <div className="mt-2 flex gap-2">
              {(["easy", "medium", "hard"] as const).map((d) => (
                <button
                  ...
                  className={`flex-1 rounded-md px-2 py-1.5 text-xs capitalize transition ${difficulty === d ? "bg-gradient-to-r from-primary to-accent text-white" : "border border-border/50 text-muted-foreground hover:text-foreground"}`}
                >
                  {d}
                </button>
              ))}
          ...
          <div className="sm:col-span-3">
            ...
            <Textarea
              ...
              placeholder="Paste notes to base questions on your material..."
            />
          </div>
        </div>
        <Button
                ...
                <div className="mt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {weakTopics.map((t) => (
                      <span
                        key={t}
                        className="rounded-full border border-destructive/40 bg-destructive/10 px-3 py-1 text-xs"
                      >
                        {t}
                      </span>
                    ))}
          ...
          {questions.map((q, i) => {
            ...
            return (
              <div key={i} className="glass rounded-2xl p-6">
                ...
                {submitted && (
                  <div
                    className={`mt-3 flex items-start gap-2 rounded-lg p-3 text-sm ${isCorrect ? "bg-success/10 text-success" : "bg-destructive/10 text-destructive"}`}
                  >
                    {isCorrect ? (
