...
function Dashboard() {
  ...
  const { data: tasks = [] } = useQuery({
    ...
    queryFn: async () =>
      (await supabase.from("tasks").select("*").order("due_date", { ascending: true }).limit(5))
        .data ?? [],
  });
  ...
  const daysToExam = nextExam
    ? Math.max(0, Math.ceil((new Date(nextExam.event_date).getTime() - Date.now()) / 86400000))
    : null;
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}>
        <div className="flex flex-wrap items-end justify-between gap-4">
          ...
          <Link
            to="/chat"
            className="glass rounded-xl px-4 py-2 text-sm flex items-center gap-2 hover:border-primary/40"
          >
        ...
        {badges.length > 0 && (
          <div className="relative mt-5 flex flex-wrap gap-2">
            {badges.slice(0, 6).map((b) => (
              <span
                key={b.badge_key}
                className="inline-flex items-center gap-1.5 rounded-full border border-warning/30 bg-warning/5 px-3 py-1 text-xs"
              >
                <Award className="h-3 w-3 text-warning" />{" "}
                {BADGE_LABELS[b.badge_key] ?? b.badge_key}
              </span>
            ))}
          </div>
        )}
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <StatCard
          icon={CalendarIcon}
          label={daysToExam !== null ? "Next exam in" : "Upcoming"}
          value={daysToExam !== null ? `${daysToExam}d` : events.length}
          sub={nextExam?.title ?? "deadlines"}
        />
      </div>
        ...
        <div className="grid gap-3 grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-8">
          {shortcuts.map((s) => (
            <Link
              key={s.to}
              to={s.to}
              className="glass hover-lift rounded-xl p-4 hover:border-primary/40"
            >
              <div
                className={`grid h-10 w-10 place-items-center rounded-lg bg-gradient-to-br ${s.color}`}
              >
      ...
      <div className="grid gap-6 lg:grid-cols-2">
          ...
          <div className="mt-4 space-y-2">
            {tasks.length === 0 && (
              <p className="text-sm text-muted-foreground py-8 text-center">
                No tasks yet. Create one from the To-Do board.
              </p>
            )}
            {tasks.map((t) => (
              <div
                key={t.id}
                className="flex items-center justify-between rounded-lg border border-border/40 bg-white/5 px-3 py-2"
              >
        ...
        <div className="glass rounded-2xl p-6">
          ...
          <div className="mt-4 space-y-3">
            {events.length === 0 && (
              <p className="text-sm text-muted-foreground py-8 text-center">
                Nothing scheduled. Add an event from the calendar.
              </p>
            )}
            {events.map((e) => (
