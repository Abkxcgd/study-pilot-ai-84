  ...
  return (
    <div className="mx-auto flex h-[calc(100vh-8rem)] max-w-4xl flex-col">
      ...
      <div className="flex-1 glass rounded-2xl p-4 overflow-y-auto">
          ...
          <div className="grid h-full place-items-center">
            <div className="max-w-md text-center">
              ...
              <div className="mt-6 grid gap-2">
                {suggestions.map((s) => (
                  <button key={s} onClick={() => send(s)} className="glass rounded-xl p-3 text-left text-sm hover:border-primary/40">
                    {s}
                  </button>
                ))}
              </div>
        ...
        {msgs.map((m, i) => (
          <div key={i} className={`flex gap-3 py-3 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
            <div className={`grid h-8 w-8 shrink-0 place-items-center rounded-lg ${m.role === "user" ? "bg-primary" : "bg-gradient-to-br from-accent to-primary"}`}>
              {m.role === "user" ? <User className="h-4 w-4" /> : <Bot className="h-4 w-4" />}
            </div>
            <div className={`max-w-[85%] rounded-2xl px-4 py-3 ${m.role === "user" ? "bg-primary text-primary-foreground" : "glass"}`}>
        ...
        {loading && (
          <div className="flex gap-3 py-3">
            <div className="grid h-8 w-8 place-items-center rounded-lg bg-gradient-to-br from-accent to-primary"><Bot className="h-4 w-4" /></div>
      ...
      <div className="mt-4 glass rounded-2xl p-2 flex gap-2 items-end">
        ...
        />
        <Button onClick={() => send()} disabled={loading || !input.trim()} size="icon" className="bg-gradient-to-r from-primary to-accent">
