...
function Page() {
  const { data } = useQuery({
    ...
    queryFn: async () => {
      ...
      const [sessions, tasks, notes, messages, stats] = await Promise.all([
        supabase.from("focus_sessions").select("duration_minutes, completed_at").gte("completed_at", since),
  ...
  const hours = days.map((d) => ({
    day: d.label,
    h: ((data?.sessions ?? []).filter((s) => s.completed_at.startsWith(d.key)).reduce((sum, s) => sum + (s.duration_minutes ?? 0), 0) / 60).toFixed(1),
  }));
  const tasksDone = days.map((d) => ({
    day: d.label,
    done: (data?.tasks ?? []).filter((t) => t.status === "done" && t.updated_at.startsWith(d.key)).length,
  }));
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <Stat icon={Flame} label="Streak" value={data?.stats?.current_streak ?? 0} sub={`Level ${data?.stats?.level ?? 1}`} accent />
      </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="glass rounded-2xl p-6 lg:col-span-2">
          ...
          {usage.length === 0 ? (
            <p className="text-sm text-muted-foreground py-8 text-center">No activity yet. Start studying to see your breakdown.</p>
          ) : (
            <ResponsiveContainer width="100%" height={280}>
              <PieChart>
                <Pie data={usage} cx="50%" cy="50%" innerRadius={60} outerRadius={100} paddingAngle={4} dataKey="value" label>
...
function Stat({ icon: Icon, label, value, sub, accent }: { icon: React.ComponentType<{ className?: string }>; label: string; value: React.ReactNode; sub: string; accent?: boolean }) {
  return (
    <div className={`glass rounded-2xl p-5 ${accent ? "glow" : ""}`}>
      ...
      <div className="text-xs text-muted-foreground flex items-center gap-1 mt-1"><TrendingUp className="h-3 w-3" />{sub}</div>
    </div>
  );
}
