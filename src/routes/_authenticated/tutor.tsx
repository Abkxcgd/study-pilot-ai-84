      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Deep explanations with real-world examples & Mermaid diagrams.
          </p>
        </div>
      </div>
        ...
        <div className="grid gap-3 sm:grid-cols-[1fr_auto]">
          <div>
            ...
            <Input
              ...
              placeholder="e.g. Backpropagation, recursion, TCP handshake"
            />
          </div>
          <div>
            <Label>Level</Label>
            <div className="mt-2 flex gap-2">
              {(["beginner", "intermediate", "advanced"] as const).map((d) => (
                <button
                  key={d}
                  onClick={() => setLevel(d)}
                  className={`rounded-md px-3 py-1.5 text-xs capitalize transition ${level === d ? "bg-gradient-to-r from-primary to-accent text-white" : "border border-border/50 text-muted-foreground hover:text-foreground"}`}
                >
                  {d}
                </button>
              ))}
