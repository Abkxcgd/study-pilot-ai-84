...
function Page() {
  ...
  const onFile = async (file: File) => {
    if (file.type.startsWith("text/") || file.name.endsWith(".txt") || file.name.endsWith(".md")) {
      setDoc(await file.text()); setDocName(file.name);
    } else {
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4">
      <div>
        <h1 className="text-3xl font-bold flex items-center gap-2"><FileSearch className="h-7 w-7 text-primary-glow" /> PDF Chat</h1>
        <p className="text-muted-foreground mt-1">Paste a document and ask questions about it.</p>
      </div>

      <div className="glass rounded-2xl p-4 space-y-3">
        <div className="flex items-center gap-2">
          <Input placeholder="Document title" value={docName} onChange={(e) => setDocName(e.target.value)} />
        ...
        <Textarea rows={6} placeholder="Paste your document text here…" value={doc} onChange={(e) => setDoc(e.target.value)} />
      ...
      <div className="glass rounded-2xl p-4 min-h-64 max-h-96 overflow-y-auto">
        {msgs.length === 0 && <p className="text-sm text-muted-foreground text-center py-10">Ask any question about your document</p>}
        {msgs.map((m, i) => (
          <div key={i} className={`py-2 ${m.role === "user" ? "text-right" : ""}`}>
            <div className={`inline-block max-w-[85%] rounded-2xl px-4 py-2 text-left ${m.role === "user" ? "bg-primary text-primary-foreground" : "glass"}`}>
      ...
      <div className="glass rounded-2xl p-2 flex gap-2">
        <Textarea rows={1} placeholder="Ask about the document…" value={input} onChange={(e) => setInput(e.target.value)}
          onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); ask(); } }}
          className="min-h-11 resize-none border-0 bg-transparent focus-visible:ring-0" />
        <Button onClick={ask} disabled={loading} size="icon" className="bg-gradient-to-r from-primary to-accent">
