...
function CalPage() {
  ...
  const create = async () => {
    ...
    setOpen(false);
    setForm({ title: "", description: "", event_date: "", event_type: "exam" });
    ...
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h1 className="text-3xl font-bold flex items-center gap-2">
            <CalendarIcon className="h-7 w-7 text-primary-glow" /> Smart Calendar
          </h1>
          <p className="text-muted-foreground mt-1">
            Track exams, assignments, and study sessions.
          </p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <Button onClick={create} className="w-full bg-gradient-to-r from-primary to-accent">
                Create
              </Button>
            </div>
          </DialogContent>
      ...
      {nextExam && (
        <div className="glass rounded-2xl p-5 flex items-center gap-4 glow">
          <div className="grid h-12 w-12 place-items-center rounded-xl bg-gradient-to-br from-primary to-accent">
            <Timer className="h-6 w-6" />
          </div>
          <div>
            <div className="text-xs text-muted-foreground">Next exam countdown</div>
            <div className="text-lg font-semibold">
              {nextExam.title} — {daysToExam} days to go
            </div>
          </div>
        </div>
      )}
          ...
          <div className="grid grid-cols-7 gap-1">
            ...
            {Array.from({ length: daysInMonth }).map((_, i) => {
              ...
              return (
                <div
                  key={day}
                  className={`aspect-square rounded-lg border border-border/30 p-1.5 text-xs ${isToday ? "bg-primary/20 border-primary" : "bg-white/5"}`}
                >
                  ...
                  <div className="mt-1 space-y-0.5">
                    {dayEvents.slice(0, 2).map((e: any) => (
                      <div
                        key={e.id}
                        className="truncate rounded bg-accent/20 text-accent px-1 text-[10px]"
                      >
                        {e.title}
                      </div>
                    ))}
